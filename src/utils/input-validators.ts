 ...
 */
export function validateColumn(column: ColumnOptionsRaw | string): void {
  ...
  if (column.maxLen !== undefined && (typeof column.maxLen !== 'number' || column.maxLen < 0)) {
    throw new TableInputError('maxLen must be a non-negative number');
  }

  if (column.minLen !== undefined && (typeof column.minLen !== 'number' || column.minLen < 0)) {
 ...
 * @throws TableInputError if validation fails
 */
export function validateRowData(rowData: Dictionary): void {
 ...
 * @throws TableInputError if validation fails
 */
export function validateColumns(columns: (ColumnOptionsRaw | string)[]): void {
 ...
 * @throws TableInputError if validation fails
 */
export function validateRows(rows: Dictionary[]): void {
