...
describe('Time Limit Performance Tests', () => {
  ...
  const createVeryLargeDataset = () => Array.from({ length: 5000 }, (_, i) => ({
    id: i + 1,
    name: `User ${i + 1}`,
    email: `user${i + 1}@example.com`,
    score: Math.floor(Math.random() * 1000),
  ...
  describe('printTable Performance', () => {
    it('should render small tables within time limit', () => {
      ...
      console.log(`Small table (${smallData.length} rows): ${duration.toFixed(2)}ms`);
    });
    ...
    it('should render medium tables within time limit', () => {
      ...
      expect(duration).toBeGreaterThan(0);
      console.log(`Medium table (${mediumData.length} rows): ${duration.toFixed(2)}ms`);
    });
    ...
    it('should render large tables within time limit', () => {
      ...
      expect(duration).toBeGreaterThan(0);
      console.log(`Large table (${largeData.length} rows): ${duration.toFixed(2)}ms`);
    });
    ...
    it('should render very large tables within time limit', () => {
      ...
      expect(duration).toBeGreaterThan(0);
      console.log(`Very large table (${veryLargeData.length} rows): ${duration.toFixed(2)}ms`);
    });
  });
  ...
  describe('Table Instance Performance', () => {
    ...
    it('should add multiple rows efficiently', () => {
      ...
      expect(duration).toBeGreaterThan(0);
      console.log(`Bulk row addition (${rows.length} rows): ${duration.toFixed(2)}ms`);
    });
  ...
  describe('Advanced Features Performance', () => {
    it('should handle column configuration efficiently', () => {
      ...
      expect(duration).toBeLessThan(TIME_LIMITS.columnConfig + TIME_LIMITS.mediumTable);
      expect(duration).toBeGreaterThan(0);
      console.log(`Column configuration with ${createMediumDataset().length} rows: ${duration.toFixed(2)}ms`);
    });
    ...
    it('should handle sorting efficiently', () => {
      ...
      expect(duration).toBeLessThan(TIME_LIMITS.sorting + TIME_LIMITS.mediumTable);
      expect(duration).toBeGreaterThan(0);
      console.log(`Sorting with ${createMediumDataset().length} rows: ${duration.toFixed(2)}ms`);
    });
    ...
    it('should handle filtering efficiently', () => {
      ...
      expect(duration).toBeLessThan(TIME_LIMITS.filtering + TIME_LIMITS.mediumTable);
      expect(duration).toBeGreaterThan(0);
      console.log(`Filtering with ${createMediumDataset().length} rows: ${duration.toFixed(2)}ms`);
    });
  });
  ...
  describe('Edge Cases Performance', () => {
    ...
    it('should handle wide datasets efficiently', () => {
      ...
      expect(duration).toBeLessThan(TIME_LIMITS.mediumTable);
      expect(duration).toBeGreaterThan(0);
      console.log(`Wide dataset (${wideData.length} rows, 10 columns): ${duration.toFixed(2)}ms`);
    });
  });
