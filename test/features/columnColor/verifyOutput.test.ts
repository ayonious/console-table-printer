...
describe('Testing column color output verification', () => {
  ...
  const verifyColoredColumn = (
    content: string,
    expectedText: string,
    expectedColor?: ColorType
  ) => {
    const trimmedContent = content.trim();

    // Verify the actual text content
    const textWithoutColors = trimmedContent
      .replace(/\x1b\[(\d+)m/g, '')
      .trim();
  ...
  it('should handle multiple colored columns', () => {
    ...
    contentLines.forEach((line) => {
      ...
      verifyColoredColumn(
        redContent,
        line.includes('123') ? '123' : 'Red text',
        'red'
      );
      verifyColoredColumn(
        greenContent,
        line.includes('456') ? '456' : 'Green text',
        'green'
      );
      verifyColoredColumn(
        blueContent,
        line.includes('789') ? '789' : 'Blue text',
        'blue'
      );
    });
  ...
  it('should handle color with different alignments', () => {
    ...
    p.printTable();
    const contentLines = getTableBody(p);
    const [, leftContent, centerContent, rightContent] =
  ...
  it('should handle column colors with row colors', () => {
    ...
    p.addRow(
      { 'red-col': 'Red text', 'blue-col': 'Blue text' },
      { color: 'green' }
    );
    ...
    contentLines.forEach((line, index) => {
      ...
      verifyColoredColumn(
        blueContent,
        line.includes('456') ? '456' : 'Blue text',
        'blue'
      );
    });
